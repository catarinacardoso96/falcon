...
AbstractNode.prototype.getParents = function() {
  ...
  for (var key in this.hostToParent) {
      for (let item in this.hostToParent[key]) {
        result.push(this.hostToParent[key][item]);
      }
  }
  return result;
};
 ...
 */
 AbstractNode.prototype.getChildren = function() {
     ...
     for (var key in this.hostToChild) {
         for (let item in this.hostToChild[key]) {
           result.push(this.hostToChild[key][item]);
         }
     }
     return result;
 };
...
AbstractNode.prototype.addChild = function(node) {
    ...
    if (!this.hostToChild[node.host])
      this.hostToChild[node.host] = [node];
    else {
      this.hostToChild[node.host].push(node);
    }
    //node.removeParentByHost(this.host);
    //node.hostToParent[this.host] = this;
    if (!node.hostToParent[this.host])
      node.hostToParent[this.host] = [this];
    else {
      node.hostToParent[this.host].push(this);
    }
...
AbstractNode.prototype.addParent = function(node) {
    ...
    if (!this.hostToParent[node.host])
      this.hostToParent[node.host] = [node];
    else {
      this.hostToParent[node.host].push(node);
    }

    //node.removeChildByHost(this.host);
    //node.hostToChild[this.host] = this;
    if (!node.hostToChild[this.host])
      node.hostToChild[this.host] = [this];
    else {
      node.hostToChild[this.host].push(this);
    }
